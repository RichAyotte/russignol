...
use russignol_signer_lib::{
    ...
    high_watermark::{MarkParams, seed_watermarks},
    server, signer,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pkh1 = *signer1.public_key_hash();
    let pkh2 = *signer2.public_key_hash();

    let params1 = MarkParams::from(signer1.secret_key());
    let params2 = MarkParams::from(signer2.secret_key());
    ...
    key_mgr.add_signer(signer1, "key1".to_string());
    key_mgr.add_signer(signer2, "key2".to_string());
    ...
    seed_watermarks(temp_dir.path(), &pkh1, 0, &params1.mac_key, demo_chain)?;
    seed_watermarks(temp_dir.path(), &pkh2, 0, &params2.mac_key, demo_chain)?;
    let mark_params = HashMap::from([(pkh1, params1), (pkh2, params2)]);
    let watermark = HighWatermark::new(temp_dir.path(), &[pkh1, pkh2], mark_params, demo_chain)?;
